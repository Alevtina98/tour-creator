...
Blockly.longStart_ = function (a, b) {
    Blockly.longStop_();
    1 == a.changedTouches.length && (Blockly.longPid_ = setTimeout(function () {
        a.button = 2;
...
Blockly.Touch.getTouchIdentifierFromEvent = function (a) {
    return a.changedTouches && a.changedTouches[0] && void 0 != a.changedTouches[0].identifier && null != a.changedTouches[0].identifier ? a.changedTouches[0].identifier : "mouse"
};
...
Blockly.VariableMap.prototype.renameVariable = function (a, b) {
    var c = this.getVariable(b), d = -1, e = -1, f = "";
    if (a || c) f = (a || c).type;
...
Blockly.VariableMap.prototype.createVariable = function (a, b, c) {
    ...
    if (d) {
        if (b && d.type != b) throw Error('Variable "' + a + '" is already in use and its type is "' + d.type + '" which conflicts with the passed in type, "' + b + '".');
...
Blockly.VariableMap.prototype.getVariable = function (a) {
    for (var b = Object.keys(this.variableMap_), c = 0; c < b.length; c++) for (var d = b[c], e = 0, f; f = this.variableMap_[d][e]; e++) if (Blockly.Names.equals(f.name, a)) return f;
    return null
};
...
Blockly.Workspace.prototype.addTopBlock = function (a) {
    ...
    if (this.isFlyout) {
        ...
        for (var b = 0, c; c = a[b]; b++) this.getVariable(c) || this.createVariable(c)
    }
};
...
Blockly.Workspace.prototype.updateVariableStore = function (a) {
    if (!this.isFlyout) {
        for (var b = Blockly.Variables.allUsedVariables(this), c = [], d = 0, e; e = b[d]; d++) {
            var f = this.getVariable(e);
            f ? c.push({name: f.name, type: f.type, id: f.getId()}) : c.push({name: e, type: null, id: null})
        }
        a && this.variableMap_.clear();
        for (d = 0; a = c[d]; d++) this.getVariable(a.name) || this.createVariable(a.name, a.type, a.id)
    }
};
Blockly.Workspace.prototype.renameVariableInternal_ = function (a, b) {
    var c = this.getVariable(b);
    if (a && c && a.type != c.type) throw Error('Variable "' + a.name + '" is type "' + a.type + '" and variable "' + b + '" is type "' + c.type + '". Both must be the same type.');
    if (c && c.name != b) var d = c.name;
    Blockly.Events.setGroup(!0);
    c = this.getAllBlocks();
    for (var e = 0; e < c.length; e++) c[e].renameVar(a.name, b), d && c[e].renameVar(d, b);
    this.variableMap_.renameVariable(a, b);
...
Blockly.Workspace.prototype.getVariableUses = function (a) {
    for (var b = [], c = this.getAllBlocks(), d = 0; d < c.length; d++) {
        ...
        if (e) for (var f = 0; f < e.length; f++) {
            var g = e[f];
            g && a && Blockly.Names.equals(g, a) && b.push(c[d])
        }
    }
    return b
};
Blockly.Workspace.prototype.deleteVariable = function (a) {
    for (var b = this.getVariableUses(a), c = 0, d; d = b[c]; c++) if ("procedures_defnoreturn" == d.type || "procedures_defreturn" == d.type) {
        b = d.getFieldValue("NAME");
        Blockly.alert(Blockly.Msg.CANNOT_DELETE_VARIABLE_PROCEDURE.replace("%1", a).replace("%2", b));
        return
    }
    var e = this, f = e.getVariable(a);
    1 < b.length ? Blockly.confirm(Blockly.Msg.DELETE_VARIABLE_CONFIRMATION.replace("%1", String(b.length)).replace("%2", a), function (a) {
        a && e.deleteVariableInternal_(f)
    }) : this.deleteVariableInternal_(f)
};
Blockly.Workspace.prototype.deleteVariableById = function (a) {
    var b = this.getVariableById(a);
    b ? this.deleteVariableInternal_(b) : console.warn("Can't delete non-existent variable: " + a)
};
Blockly.Workspace.prototype.deleteVariableInternal_ = function (a) {
    var b = this.getVariableUses(a.name);
    Blockly.Events.setGroup(!0);
    for (var c = 0; c < b.length; c++) b[c].dispose(!0, !1);
    this.variableMap_.deleteVariable(a);
    Blockly.Events.setGroup(!1)
};
...
Blockly.Workspace.prototype.getVariable = function (a) {
    return this.variableMap_.getVariable(a)
};
...
Blockly.Bubble.prototype.layoutBubble_ = function () {
    var a = -this.width_ / 4, b = -this.height_ - Blockly.BlockSvg.MIN_BLOCK_Y, c = this.workspace_.getMetrics();
...
Blockly.Bubble.prototype.renderArrow_ = function () {
    ...
    if (b == d && c == e) a.push("M " + b + "," + c); else {
        ...
        var k = Math.sin(h), n = Math.cos(h), p = this.getBubbleSize();
...
Blockly.Connection.prototype.isConnectionAllowed = function (a) {
    ...
    if (a.type == Blockly.OUTPUT_VALUE || a.type == Blockly.PREVIOUS_STATEMENT) if (a.isConnected() || this.isConnected()) return !1;
...
Blockly.Connection.prototype.respawnShadow_ = function () {
    ...
    if (a.workspace && b && Blockly.Events.recordUndo) if (a = Blockly.Xml.domToBlock(b, a.workspace), a.outputConnection) this.connect(a.outputConnection); else if (a.previousConnection) this.connect(a.previousConnection); else throw"Child block does not have output or previous statement.";
};
...
Blockly.Connection.prototype.toString = function () {
    ...
    if (a) if (a.outputConnection == this) var b = "Output Connection of "; else if (a.previousConnection == this) b = "Previous Connection of "; else if (a.nextConnection == this) b = "Next Connection of "; else if (b = goog.array.find(a.inputList, function (a) {
        return a.connection == this
    }, this)) b = 'Input "' + b.name + '" connection on '; else return console.warn("Connection not actually connected to sourceBlock_"), "Orphan Connection"; else return "Orphan Connection";
...
Blockly.Extensions.buildTooltipForDropdown = function (a, b) {
    var c = [];
    document && Blockly.utils.runAfterPageLoad(function () {
    ...
    return function () {
        ...
        this.setTooltip(function () {
            ...
            null == e ? -1 === c.indexOf(this.type) && (d = "No tooltip mapping for value " + d + " of field " + a, null != this.type && (d += " of block type " + this.type),
...
Blockly.RenderedConnection.prototype.hideAll = function () {
    this.setHidden(!0);
    if (this.targetConnection) for (var a = this.targetBlock().getDescendants(), b = 0; b < a.length; b++) {
...
Blockly.DraggedConnectionManager.prototype.initAvailableConnections_ = function () {
    var a = this.topBlock_.getConnections_(!1), b = this.topBlock_.lastConnectionInStack_();
...
Blockly.DraggedConnectionManager.prototype.updateClosest_ = function (a) {
    ...
    for (var c = 0; c < this.availableConnections_.length; c++) {
        var d = this.availableConnections_[c], e = d.closest(this.radiusConnection_, a);
...
Blockly.WorkspaceDragger.prototype.drag = function (a) {
    var b = this.startDragMetrics_, c = goog.math.Coordinate.sum(this.startScrollXY_, a);
...
Blockly.Gesture = function (a, b) {
    ...
    this.startWorkspace_ = this.targetBlock_ = this.startBlock_ = this.startField_ = null;
    this.creatorWorkspace_ = b;
    this.isDraggingBlock_ = this.isDraggingWorkspace_ = this.hasExceededDragRadius_ = !1;
    this.mostRecentEvent_ = a;
    this.flyout_ = this.workspaceDragger_ = this.blockDragger_ = this.onUpWrapper_ = this.onMoveWrapper_ = null;
...
Blockly.Gesture.prototype.handleMove = function (a) {
    ...
    this.isDraggingWorkspace_ ? this.workspaceDragger_.drag(this.currentDragDeltaXY_) : this.isDraggingBlock_ && this.blockDragger_.dragBlock(this.mostRecentEvent_, this.currentDragDeltaXY_);
    a.preventDefault();
    a.stopPropagation()
};
Blockly.Gesture.prototype.handleUp = function (a) {
    this.updateFromEvent_(a);
    Blockly.longStop_();
    this.isEnding_ ? console.log("Trying to end a gesture recursively.") : (this.isEnding_ = !0, this.isDraggingBlock_ ? this.blockDragger_.endBlockDrag(a, this.currentDragDeltaXY_) : this.isDraggingWorkspace_ ? this.workspaceDragger_.endDrag(this.currentDragDeltaXY_) : this.isFieldClick_() ? this.doFieldClick_() : this.isBlockClick_() ? this.doBlockClick_() : this.isWorkspaceClick_() && this.doWorkspaceClick_(), a.preventDefault(), a.stopPropagation(),
        this.dispose())
};
Blockly.Gesture.prototype.cancel = function () {
    this.isEnding_ || (Blockly.longStop_(), this.isDraggingBlock_ ? this.blockDragger_.endBlockDrag(this.mostRecentEvent_, this.currentDragDeltaXY_) : this.isDraggingWorkspace_ && this.workspaceDragger_.endDrag(this.currentDragDeltaXY_), this.dispose())
};
...
Blockly.ConnectionDB.prototype.findPositionForConnection_ = function (a) {
    ...
    for (var b = 0, c = this.length; b < c;) {
        ...
        if (this[d].y_ < a.y_) b = d + 1; else if (this[d].y_ > a.y_) c = d; else {
            b = d;
            break
        }
    }
...
Blockly.ConnectionDB.prototype.searchForClosest = function (a, b, c) {
    if (!this.length) return {connection: null, radius: b};
    var d = a.y_, e = a.x_;
    a.x_ = e + c.x;
    a.y_ = d + c.y;
...
Blockly.utils.getInjectionDivXY_ = function (a) {
    for (var b = 0, c = 0; a;) {
        var d = Blockly.utils.getRelativeXY(a), e = Blockly.utils.getScale_(a);
        b = b * e + d.x;
        c = c * e + d.y;
...
Blockly.Grid.createDom = function (a, b, c) {
    ...
    0 < b.length && 0 < b.spacing && (Blockly.utils.createSvgElement("line", {stroke: b.colour}, a), 1 < b.length && Blockly.utils.createSvgElement("line", {stroke: b.colour}, a));
    return a
};
Blockly.Options = function (a) {
    var b = !!a.readOnly;
    if (b) var c = null, d = !1, e = !1, f = !1, g = !1, h = !1,
...
Blockly.Scrollbar = function (a, b, c, d) {
    ...
    b ? (this.svgBackground_.setAttribute("height", Blockly.Scrollbar.scrollbarThickness), this.outerSvg_.setAttribute("height", Blockly.Scrollbar.scrollbarThickness), this.svgHandle_.setAttribute("height", Blockly.Scrollbar.scrollbarThickness - 5), this.svgHandle_.setAttribute("y", 2.5), this.lengthAttribute_ = "width", this.positionAttribute_ =
        "x") : (this.svgBackground_.setAttribute("width", Blockly.Scrollbar.scrollbarThickness), this.outerSvg_.setAttribute("width", Blockly.Scrollbar.scrollbarThickness), this.svgHandle_.setAttribute("width", Blockly.Scrollbar.scrollbarThickness - 5), this.svgHandle_.setAttribute("x", 2.5), this.lengthAttribute_ = "height", this.positionAttribute_ = "y");
    ...
    this.onMouseDownHandleWrapper_ = Blockly.bindEventWithChecks_(this.svgHandle_,
        "mousedown", this, this.onMouseDownHandle_)
};
...
Blockly.Xml.blockToDom = function (a, b) {
    ...
    d = 0;
    for (var e; e = a.inputList[d]; d++) for (var f = 0, g; g = e.fieldRow[f]; f++) {
        var h = g;
        h.name && h.EDITABLE && (g = goog.dom.createDom("field", null, h.getValue()), g.setAttribute("name", h.name), h instanceof Blockly.FieldVariable && (h = a.workspace.getVariable(h.getValue())) &&
        (g.setAttribute("id", h.getId()), g.setAttribute("variabletype", h.type)), c.appendChild(g))
    }
    if (d = a.getCommentText()) d = goog.dom.createDom("comment", null, d), "object" == typeof a.comment && (d.setAttribute("pinned", a.comment.isVisible()), e = a.comment.getBubbleSize(), d.setAttribute("h", e.height), d.setAttribute("w", e.width)), c.appendChild(d);
    a.data && (d = goog.dom.createDom("data", null, a.data), c.appendChild(d));
    for (d = 0; e = a.inputList[d]; d++) {
        var k;
        g = !0;
        e.type != Blockly.DUMMY_INPUT && (h = e.connection.targetBlock(), e.type ==
        Blockly.INPUT_VALUE ? k = goog.dom.createDom("value") : e.type == Blockly.NEXT_STATEMENT && (k = goog.dom.createDom("statement")), f = e.connection.getShadowDom(), !f || h && h.isShadow() || k.appendChild(Blockly.Xml.cloneShadow_(f)), h && (k.appendChild(Blockly.Xml.blockToDom(h, b)), g = !1), k.setAttribute("name", e.name), g || c.appendChild(k))
    }
    ...
    a.isDeletable() ||
    a.isShadow() || c.setAttribute("deletable", !1);
    a.isMovable() || a.isShadow() || c.setAttribute("movable", !1);
    a.isEditable() || c.setAttribute("editable", !1);
    if (d = a.getNextBlock()) k = goog.dom.createDom("next", null, Blockly.Xml.blockToDom(d, b)), c.appendChild(k);
...
Blockly.Xml.domToWorkspace = function (a, b) {
    ...
    try {
        for (var h = 0; h < e; h++) {
            var k = a.childNodes[h], n = k.nodeName.toLowerCase();
...
Blockly.Xml.domToBlock = function (a, b) {
    ...
    try {
        ...
        if (b.rendered) {
            ...
            for (var e = d.getDescendants(), f = e.length - 1; 0 <= f; f--) e[f].initSvg();
...
Blockly.Xml.domToBlockHeadless_ = function (a, b) {
    ...
    c = b.newBlock(d, e);
    for (var f = null, g = 0, h; h = a.childNodes[g]; g++) if (3 != h.nodeType) {
        var k = null, n = null;
        f = 0;
        for (var p; p = h.childNodes[f]; f++) 1 == p.nodeType && ("block" == p.nodeName.toLowerCase() ? k = p : "shadow" == p.nodeName.toLowerCase() && (n = p));
        !k && n && (k = n);
        f = h.getAttribute("name");
        switch (h.nodeName.toLowerCase()) {
            case "mutation":
                c.domToMutation &&
                (c.domToMutation(h), c.initSvg && c.initSvg());
                break;
            case "comment":
                ...
                !isNaN(f) && !isNaN(h) && c.comment && c.comment.setVisible && c.comment.setBubbleSize(f, h);
                break;
            case "data":
            ...
            case "statement":
                ...
                k && (f = Blockly.Xml.domToBlockHeadless_(k, b), f.outputConnection ? h.connection.connect(f.outputConnection) : f.previousConnection ? h.connection.connect(f.previousConnection) : goog.asserts.fail("Child block does not have output or previous statement."));
                break;
            case "next":
                n && c.nextConnection && c.nextConnection.setShadowDom(n);
                k && (goog.asserts.assert(c.nextConnection, "Next statement does not exist."), goog.asserts.assert(!c.nextConnection.isConnected(), "Next statement is already connected."),
                    f = Blockly.Xml.domToBlockHeadless_(k, b), goog.asserts.assert(f.previousConnection, "Next block does not have previous statement."), c.nextConnection.connect(f.previousConnection));
                break;
            default:
                console.warn("Ignoring unknown tag: " + h.nodeName)
        }
    }
    (g = a.getAttribute("inline")) && c.setInputsInline("true" == g);
    (g = a.getAttribute("disabled")) && c.setDisabled("true" == g);
    (g = a.getAttribute("deletable")) && c.setDeletable("true" == g);
    (g = a.getAttribute("movable")) && c.setMovable("true" == g);
    (g = a.getAttribute("editable")) &&
...
Blockly.ZoomControls.prototype.createDom = function () {
    ...
    var c = Blockly.utils.createSvgElement("clipPath", {id: "blocklyZoomoutClipPath" + b}, this.svgGroup_);
    Blockly.utils.createSvgElement("rect", {width: 32, height: 32, y: 77}, c);
    ...
    c = Blockly.utils.createSvgElement("clipPath", {id: "blocklyZoominClipPath" + b}, this.svgGroup_);
    Blockly.utils.createSvgElement("rect", {width: 32, height: 32, y: 43}, c);
    ...
    c = Blockly.utils.createSvgElement("clipPath", {id: "blocklyZoomresetClipPath" + b}, this.svgGroup_);
...
Blockly.WorkspaceSvg.prototype.setupDragSurface = function () {
    if (this.useWorkspaceDragSurface_ && !this.isDragSurfaceActive_) {
        ...
        var a = this.svgBlockCanvas_.previousSibling, b = this.getParentSvg().getAttribute("width"),
            c = this.getParentSvg().getAttribute("height"), d = Blockly.utils.getRelativeXY(this.svgBlockCanvas_);
            ...
            if (!isNaN(c) && !isNaN(d)) {
                ...
                do {
                    ...
                    if (!a) {
                        ...
                        f = 0;
                        for (var n; n = k[f]; f++) if (n.closest(Blockly.SNAP_RADIUS, new goog.math.Coordinate(c, d)).connection) {
                            a = !0;
                            break
                        }
                    }
...
Blockly.WorkspaceSvg.prototype.refreshToolboxSelection_ = function () {
    this.toolbox_ && this.toolbox_.flyout_ && !this.currentGesture_ && this.toolbox_.refreshSelection()
};
Blockly.WorkspaceSvg.prototype.renameVariable = function (a, b) {
    Blockly.WorkspaceSvg.superClass_.renameVariable.call(this, a, b);
...
Blockly.WorkspaceSvg.prototype.deleteVariable = function (a) {
    Blockly.WorkspaceSvg.superClass_.deleteVariable.call(this, a);
    this.refreshToolboxSelection_()
};
...
Blockly.WorkspaceSvg.prototype.onMouseWheel_ = function (a) {
    this.currentGesture_ && this.currentGesture_.cancel();
    var b = -a.deltaY / 50, c = Blockly.utils.mouseToSvg(a, this.getParentSvg(), this.getInverseScreenCTM());
    this.zoom(c.x, c.y, b);
    a.preventDefault()
};
Blockly.WorkspaceSvg.prototype.getBlocksBoundingBox = function () {
    ...
    return {
        x: b.topLeft.x, y: b.topLeft.y, width: b.bottomRight.x -
            b.topLeft.x, height: b.bottomRight.y - b.topLeft.y
    }
};
...
Blockly.WorkspaceSvg.prototype.zoom = function (a, b, c) {
    var d = this.options.zoomOptions.scaleSpeed, e = this.getMetrics(), f = this.getParentSvg().createSVGPoint();
    f.x = a;
    f.y = b;
...
Blockly.WorkspaceSvg.prototype.zoomToFit = function () {
    var a = this.getMetrics(), b = this.getBlocksBoundingBox(), c = b.width;
    b = b.height;
    if (c) {
        var d = a.viewWidth, e = a.viewHeight;
...
Blockly.WorkspaceSvg.prototype.scrollCenter = function () {
    if (this.scrollbar) {
        var a = this.getMetrics(), b = (a.contentWidth - a.viewWidth) / 2;
...
Blockly.WorkspaceSvg.getTopLevelWorkspaceMetrics_ = function () {
    ...
    if (this.toolbox_) if (this.toolboxPosition == Blockly.TOOLBOX_AT_TOP || this.toolboxPosition == Blockly.TOOLBOX_AT_BOTTOM) c.height -= a.height; else if (this.toolboxPosition == Blockly.TOOLBOX_AT_LEFT || this.toolboxPosition == Blockly.TOOLBOX_AT_RIGHT) c.width -= a.width;
...
Blockly.WorkspaceSvg.prototype.getGesture = function (a) {
    var b = "mousedown" == a.type || "touchstart" == a.type, c = this.currentGesture_;
...
Blockly.Mutator.prototype.createEditor_ = function () {
    ...
    if (this.quarkNames_.length) for (var a = goog.dom.createDom("xml"), b = 0, c; c = this.quarkNames_[b]; b++) a.appendChild(goog.dom.createDom("block", {type: c})); else a = null;
    a = {
        languageTree: a,
...
Blockly.Mutator.prototype.workspaceChanged_ = function () {
    if (!this.workspace_.isDragging()) for (var a = this.workspace_.getTopBlocks(!1), b = 0, c; c = a[b]; b++) {
...
Blockly.Mutator.prototype.getFlyoutMetrics_ = function () {
    return {viewHeight: this.workspaceHeight_, viewWidth: this.workspaceWidth_, absoluteTop: 0, absoluteLeft: 0}
};
...
Blockly.Block = function (a, b, c) {
    ...
    Blockly.Events.isEnabled() && Blockly.Events.fire(new Blockly.Events.BlockCreate(this));
...
Blockly.Block.prototype.unplug = function (a) {
    if (this.outputConnection) this.outputConnection.isConnected() && this.outputConnection.disconnect(); else if (this.previousConnection) {
        var b = null;
...
Blockly.Block.prototype.setColour = function (a) {
    var b = Number(a);
    if (!isNaN(b) && 0 <= b && 360 >= b) this.hue_ = b, this.colour_ = Blockly.hueToRgb(b); else if (goog.isString(a) && /^#[0-9a-fA-F]{6}$/.test(a)) this.colour_ = a, this.hue_ = null; else throw"Invalid colour: " + a;
};
...
Blockly.Block.prototype.getVars = function () {
    for (var a = [], b = 0, c; c = this.inputList[b]; b++) for (var d = 0, e; e = c.fieldRow[d]; d++) e instanceof Blockly.FieldVariable && a.push(e.getValue());
    return a
};
Blockly.Block.prototype.renameVar = function (a, b) {
    for (var c = 0, d; d = this.inputList[c]; c++) for (var e = 0, f; f = d.fieldRow[e]; e++) f instanceof Blockly.FieldVariable && Blockly.Names.equals(a, f.getValue()) && f.setValue(b)
};
    ...
    for (g = 0; g < a.length; g++) if (e = a[g], "string" == typeof e) b.push([e, void 0]); else {
        ...
        do if (h = !1, "string" == typeof e) f = new Blockly.FieldLabel(e); else switch (e.type) {
            ...
            case "input_dummy":
                ...
                break;
            case "field_label":
                f = Blockly.Block.newFieldLabelFromJson_(e);
                break;
            case "field_input":
                f = Blockly.Block.newFieldTextInputFromJson_(e);
                break;
            case "field_angle":
            ...
            case "field_colour":
                ...
                break;
            case "field_variable":
                f = Blockly.Block.newFieldVariableFromJson_(e);
                break;
            case "field_dropdown":
                f =
                    new Blockly.FieldDropdown(e.options);
                break;
            case "field_image":
                f = Blockly.Block.newFieldImageFromJson_(e);
                break;
            case "field_number":
...
Blockly.Block.newFieldImageFromJson_ = function (a) {
...
Blockly.ContextMenu.blockDeleteOption = function (a) {
    var b = a.getDescendants(!0).length, c = a.getNextBlock();
...
Blockly.BlockSvg.prototype.moveBy = function (a, b) {
    goog.asserts.assert(!this.parentBlock_, "Block has parent.");
    var c = new Blockly.Events.BlockMove(this), d = this.getRelativeToSurfaceXY();
    ...
    if (!this.workspace.options.readOnly && this.contextMenu) {
        ...
        if (this.isDeletable() && this.isMovable() && !b.isInFlyout) {
            ...
            this.workspace.options.collapse && (this.collapsed_ ? (d = {enabled: !0}, d.text = Blockly.Msg.EXPAND_BLOCK, d.callback = function () {
                b.setCollapsed(!1)
            }) : (d = {enabled: !0}, d.text = Blockly.Msg.COLLAPSE_BLOCK, d.callback = function () {
                b.setCollapsed(!0)
            }), c.push(d));
            this.workspace.options.disable && (d = {
                text: this.disabled ? Blockly.Msg.ENABLE_BLOCK : Blockly.Msg.DISABLE_BLOCK,
                enabled: !this.getInheritedDisabled(), callback: function () {
...
Blockly.BlockSvg.prototype.disposeUiEffect = function () {
    ...
    var a = this.workspace.getSvgXY(this.svgGroup_), b = this.svgGroup_.cloneNode(!0);
    b.translateX_ = a.x;
    b.translateY_ = a.y;
...
Blockly.BlockSvg.prototype.updateColour = function () {
    if (!this.disabled) {
        var a = this.getColour(), b = goog.color.hexToRgb(a);
        if (this.isShadow()) b = goog.color.lighten(b, .6), a = goog.color.rgbArrayToHex(b), this.svgPathLight_.style.display = "none", this.svgPathDark_.setAttribute("fill", a); else {
...
Blockly.Events.Create.prototype.run = function (a) {
    ...
    if (a) a = goog.dom.createDom("xml"), a.appendChild(this.xml), Blockly.Xml.domToWorkspace(a, b); else {
        a = 0;
...
Blockly.FieldAngle.prototype.updateGraph_ = function () {
    if (this.gauge_) {
        var a = Number(this.getText()) + Blockly.FieldAngle.OFFSET, b = goog.math.toRadians(a);
        a = ["M ", Blockly.FieldAngle.HALF, ",", Blockly.FieldAngle.HALF];
        var c = Blockly.FieldAngle.HALF, d = Blockly.FieldAngle.HALF;
        if (!isNaN(b)) {
            var e = goog.math.toRadians(Blockly.FieldAngle.OFFSET), f = Math.cos(e) * Blockly.FieldAngle.RADIUS,
...
Blockly.FieldColour.prototype.showEditor_ = function () {
    ...
    var a = Blockly.utils.getViewportBBox(), b = this.getScaledBBox_(), c = this.createWidget_(),
...
Blockly.Variables.allUsedVariables = function (a) {
    if (a instanceof Blockly.Block) var b = a.getDescendants(); else if (a.getAllBlocks) b = a.getAllBlocks(); else throw"Not Block or Workspace: " + a;
...
Blockly.Variables.allVariables = function (a) {
    return a instanceof Blockly.Block ? (console.warn("Deprecated call to Blockly.Variables.allVariables with a block instead of a workspace.  You may want Blockly.Variables.allUsedVariables"), {}) : a.getAllVariables()
};
...
Blockly.Variables.renameVariable = function (a, b, c) {
    var d = function (e) {
        Blockly.Variables.promptName(Blockly.Msg.RENAME_VARIABLE_TITLE.replace("%1", b.name), e, function (e) {
            if (e) {
...
Blockly.FieldVariable.prototype.setValue = function (a) {
    var b = a, c = a;
    if (this.sourceBlock_) {
        var d = this.sourceBlock_.workspace.getVariableById(a);
        if (d) c = d.name; else if (d = this.sourceBlock_.workspace.getVariable(a)) b = d.getId();
        Blockly.Events.isEnabled() && Blockly.Events.fire(new Blockly.Events.BlockChange(this.sourceBlock_, "field", this.name, this.value_, b))
    }
    this.value_ = b;
    this.setText(c)
};
...
Blockly.FieldVariable.dropdownCreate = function () {
    var a = [], b = this.getText(), c = b ? !0 : !1, d = null;
    this.sourceBlock_ && (d = this.sourceBlock_.workspace);
    if (d) {
        var e = this.getVariableTypes_();
        a = [];
        for (var f = 0; f < e.length; f++) {
            var g = d.getVariablesOfType(e[f]);
            a = a.concat(g)
        }
        for (f = 0; f < a.length; f++) if (c && goog.string.caseInsensitiveEquals(a[f].name, b)) {
            c = !1;
            break
        }
    }
    c && d && (f = d.createVariable(b), a.push(f));
    a.sort(Blockly.VariableModel.compareByName);
    c = [];
    for (f = 0; f < a.length; f++) c[f] = [a[f].name, a[f].getId()];
    c.push([Blockly.Msg.RENAME_VARIABLE,
...
Blockly.Generator.prototype.init = void 0;
Blockly.Generator.prototype.scrub_ = void 0;
Blockly.Generator.prototype.finish = void 0;
Blockly.Generator.prototype.scrubNakedValue = void 0;
...
Blockly.Flyout.prototype.init = function (a) {
    ...
    this.workspace_.getVariable = this.targetWorkspace_.getVariable.bind(this.targetWorkspace_);
    this.workspace_.getVariableById = this.targetWorkspace_.getVariableById.bind(this.targetWorkspace_);
    this.workspace_.getVariablesOfType = this.targetWorkspace_.getVariablesOfType.bind(this.targetWorkspace_);
    this.workspace_.deleteVariable = this.targetWorkspace_.deleteVariable.bind(this.targetWorkspace_);
    this.workspace_.deleteVariableById = this.targetWorkspace_.deleteVariableById.bind(this.targetWorkspace_);
    this.workspace_.renameVariable = this.targetWorkspace_.renameVariable.bind(this.targetWorkspace_);
    this.workspace_.renameVariableById = this.targetWorkspace_.renameVariableById.bind(this.targetWorkspace_)
};
...
Blockly.Flyout.prototype.createBlock = function (a) {
    ...
    Blockly.Events.isEnabled() && (Blockly.Events.setGroup(!0), Blockly.Events.fire(new Blockly.Events.Create(b)));
...
Blockly.HorizontalFlyout.prototype.position = function () {
    if (this.isVisible()) {
        ...
        if (a) {
            ...
            var b = a.absoluteLeft, c = a.absoluteTop;
...
Blockly.VerticalFlyout.prototype.position = function () {
    if (this.isVisible()) {
        ...
        if (a) {
            ...
            var b = a.absoluteTop, c = a.absoluteLeft;
...
Blockly.VerticalFlyout.prototype.reflowInternal_ = function () {
    ...
    if (this.width_ != a) {
        ...
        if (this.RTL) for (c = 0; d = this.buttons_[c]; c++) b = d.getPosition().y, d.moveTo(a - d.width - this.MARGIN - Blockly.BlockSvg.TAB_WIDTH, b);
        this.width_ = a;
        this.targetWorkspace_.resize()
    }
};
    ...
    if (!Blockly.mainWorkspace.options.readOnly && !Blockly.utils.isTargetInput(a)) {
        ...
        if (27 == a.keyCode) Blockly.hideChaff(); else if (8 == a.keyCode || 46 == a.keyCode) {
            ...
            Blockly.selected && Blockly.selected.isDeletable() && Blockly.selected.isMovable() && (67 == a.keyCode ? (Blockly.hideChaff(),
                Blockly.copy_(Blockly.selected)) : 88 == a.keyCode && (Blockly.copy_(Blockly.selected), b = !0));
            86 == a.keyCode ? Blockly.clipboardXml_ && (Blockly.Events.setGroup(!0), Blockly.clipboardSource_.paste(Blockly.clipboardXml_), Blockly.Events.setGroup(!1)) : 90 == a.keyCode && (Blockly.hideChaff(), Blockly.mainWorkspace.undo(a.shiftKey))
        }
        b && (Blockly.Events.setGroup(!0), Blockly.hideChaff(), Blockly.selected.dispose(!0, !0), Blockly.Events.setGroup(!1))
    }
};
...
Blockly.bindEventWithChecks_ = function (a, b, c, d, e, f) {
    ...
    var k = [[a, b, h]];
    if (b in Blockly.Touch.TOUCH_MAP) for (var n = function (a) {
        h(a);
        var b = !f;
...
Blockly.bindEvent_ = function (a, b, c, d) {
    ...
    var f = [[a, b, e]];
    if (b in Blockly.Touch.TOUCH_MAP) for (var g = function (a) {
